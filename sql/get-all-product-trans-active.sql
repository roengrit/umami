select all_table.* ,product.product_type from (
    ...
    union all 
			 select "order".created_at,product_id,qty,price,average_cost,"order"."flag",doc_type,order_sub.i_d,'order_sub' as tb   
			 from order_sub join "order" on order_sub.doc_no = "order".doc_no where "order".active and  order_sub.active and product_id = 1  
    union all 
			 select "pick_up".created_at,product_id,qty,price,average_cost,"pick_up"."flag",doc_type,pick_up_sub.i_d,'pick_up_sub' as tb   
...
ORDER BY all_table.created_at,all_table.i_d